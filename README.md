...
当前为开放演示：登录页面保持不变，用户与管理员均可直接点击登录，无真实微信授权或管理员资格核验。会员绑定输入任意内容（包括留空）都可进入；有效名册资料仍正常绑定，其他输入使用虚拟演示身份，不将任意输入写入会员名册。已占用或停用的会员不被抢绑或启用，必要时创建独立的虚拟访客。此便利入口仅在浏览器演示层使用，领域层六位编号、绑定及预约规则仍严格校验，正式接入必须移除该入口。
